...
const goalTypes: { key: GoalType; label: string; helper: string }[] = [
  {
    key: "lose-weight",
    label: "Lose weight",
    helper: "small calorie deficit, protein-forward",
  },
  {
    key: "maintain",
    label: "Maintain",
    helper: "stability and energy balance",
  },
  {
    key: "gain-muscle",
    label: "Gain muscle",
    helper: "protein + small surplus",
  },
  {
    key: "improve-consistency",
    label: "Consistency",
    helper: "log first, optimize later",
  },
];
...
export default function GoalsScreen() {
  ...
  const sampleInsight = goal
    ? recommendWeeklyAdjustment({
        goal,
        checkIn: {
          weekStart: "preview",
          averageCalories: goal.calories - 50,
          averageProteinG: goal.proteinTargetG * 0.92,
  ...
  return (
    <ScrollView style={styles.fill} contentContainerStyle={styles.container}>
      ...
      <Text style={styles.subtitle}>
        Howdy Morning goals use Apple Health context when available, but stay
        boring-on-purpose: calories, protein, trend, consistency.
      </Text>
        ...
        {editingCalories ? (
          ...
          <Pressable onPress={startEditingCalories}>
            <Text style={styles.big}>
              {Math.round(goal?.calories ?? 0)} kcal
            </Text>
          </Pressable>
        )}
        <Text style={styles.macro}>
          {Math.round(goal?.proteinTargetG ?? 0)}g protein ·{" "}
          {Math.round(goal?.carbsG ?? 0)}g carbs · {Math.round(goal?.fatG ?? 0)}
          g fat
        </Text>
        <Text style={styles.disclaimer}>
          Estimates based on general sports-nutrition research (e.g. ISSN
          protein guidance). Not medical or dietitian advice — check with a
          ...
          <Pressable
            onPress={
              profile?.calorieOverride
                ? resetCalorieOverride
                : startEditingCalories
            }
          >
            <Text style={styles.editLink}>
              {profile?.calorieOverride
                ? "Manually set · tap to reset to calculated"
                : "Tap to edit"}
            </Text>
          </Pressable>
        )}
      ...
      {goalTypes.map((item) => (
        <Pressable
          ...
          style={[
            styles.goalRow,
            profile?.goalType === item.key && styles.goalRowActive,
          ]}
        >
          <View>
            <Text
              style={[
                styles.goalLabel,
                profile?.goalType === item.key && styles.goalLabelActive,
              ]}
            >
              {item.label}
            </Text>
            <Text
              style={[
                styles.goalHelper,
                profile?.goalType === item.key && styles.goalHelperActive,
              ]}
            >
              {item.helper}
            </Text>
      ...
      <View style={styles.insight}>
        ...
        <Text style={styles.insightDelta}>
          Suggested delta: {sampleInsight?.recommendedCalorieDelta ?? 0} kcal
        </Text>
      </View>
    </ScrollView>
  );
